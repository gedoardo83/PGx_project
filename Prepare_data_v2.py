...
#Read CYP2D6 AF
cyp2d6_af_table = pd.read_csv(cyp2d6_af_file, sep="\t", header=0, index_col='Diplotype')
...
CYP_alleles_table = pd.read_csv(args.cyp_alleles, sep="\t", encoding = "ISO-8859-1", usecols=['sample ID','CYP2D6','CYP2C19'], converters={'CYP2D6':not_curly})
...
#Extract single diplotypes and select the one with the highest frequency
col_idx = CYP_alleles_table.columns.get_loc('CYP2D6')
for idx, value in enumerate(CYP_alleles_table['CYP2D6'].values):
    genos = value.split(', ')
    df_max = []
    for g in genos:
        g = re.sub(r'x[3-9]+','>=3', g)
        g = re.sub(r'4x2+','4>=2', g)
        f_max = cyp2d6_af_table.at[g,POP]
        df_max.append(f_max)
    sort_idx = sorted(range(len(df_max)), key=lambda k: df_max[k], reverse = True)
        ...
        if m.group(2) == "2":
            if geno in CYP2D6_pheno['Diplotype'].values:
                ...
                if geno_new in CYP2D6_pheno['Diplotype'].values:
                    ...
                    CYP_alleles_table.iloc[idx,col_idx]="UNDETERMINED1"
        elif m.group(2) > 2:
            ...
            if geno_new in CYP2D6_pheno['Diplotype'].values:
                ...
                if geno_new in CYP2D6_pheno['Diplotype'].values:
                    ...
                    print("CYP2D6 dyplotype not found:", geno)
                    missed_diplotypes.append(geno)
                    CYP_alleles_table.iloc[idx,col_idx]="UNDETERMINED2"
    else:
        if geno in CYP2D6_pheno['Diplotype'].values:
            CYP_alleles_table.iloc[idx,col_idx]=CYP2D6_pheno.loc[CYP2D6_pheno['Diplotype']==geno, 'Phenotype'].values[0]
        else:
            print("CYP2D6 dyplotype not found:", geno)
            missed_diplotypes.append(geno)
            CYP_alleles_table.iloc[idx,col_idx]="UNDETERMINED3"
